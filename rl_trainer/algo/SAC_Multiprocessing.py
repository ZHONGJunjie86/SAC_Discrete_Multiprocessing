...
class SAC:
    def __init__(self, obs_dim, act_dim, num_agent, args,device):
        ...
        if self.policy_type == "Gaussian":
            ...
            if self.automatic_entropy_tuning is True:
                self.log_alpha = torch.tensor(-0.5, requires_grad=True, device=self.device) #-np.log(12)*np.e
                self.target_entropy = -0.98*np.log(1/3)#0.1#-3##-12# #np.log(3)#-np.log(1/3)#action_space.shape
    ...
    def update(self,new_lr):
        ...
        if len(self.replay_buffer) < 32768 :#self.batch_size:#25e3
            return 0, 0

        #k = 1.0 + len(self.replay_buffer) / 9e5
        #batch_size_ = int(self.batch_size * k)
        train_steps = 2#int(k * 4)# Sample a batch from memory
        train_steps_critic = 66
        

        for i in range(train_steps_critic):
            ...
            with torch.no_grad():  # qf_next 里面带了下一状态的熵
                ...
                #print("min_qf_next_target.size()",min_qf_next_target.size())
                next_q_value = reward_batch +  ( self.gamma*min_qf_next_target).reshape(self.batch_size,1,3)
                #print("next_q_value.size()",next_q_value.size())   #(1- mask_batch) *
            ...
            ####################################
            if i % 2 == 0 or self.total_it>=1000:
            
                _, log_pi, action_probs = self.actor.sample(state_batch)

                qf1_pi, qf2_pi = self.critic(state_batch)  #因为s是旧策略采集的，所以要reparameterize
                min_qf_pi = torch.sum((torch.min(qf1_pi, qf2_pi)).reshape(self.batch_size,3,4)*action_probs.reshape(self.batch_size,3,4),dim=-1,keepdim = True)
                #print("min_qf_pi.size()",min_qf_pi.size())
                entropies = -torch.sum(action_probs.reshape(self.batch_size,3,4)*log_pi.reshape(self.batch_size,3,4),dim=-1,keepdim=True)
                ...
                if self.automatic_entropy_tuning:
                    ...
                    alpha_loss = -torch.mean(self.log_alpha * (self.target_entropy-entropies.detach()).detach())
...
class Worker(nn.Module):
    ...
    def forward(self, tensor_cv):
        ...
        x = F.relu(self.conv4(x)).reshape(self.batch_size,1,640)
        x = self.linear(x)
