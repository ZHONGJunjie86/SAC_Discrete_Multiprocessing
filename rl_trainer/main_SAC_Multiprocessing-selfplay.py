...
def train(rank,args,device,log_dir,run_dir,shared_lock,
    ...
    model_actor_path =  "/home/j-zhong/work_place/TD3_SAC_PPO_multi_Python/rl_trainer/models/snakes_3v3/run24/trained_model/actor_4000.pth"
    model_actor_path_new =  "/home/j-zhong/work_place/TD3_SAC_PPO_multi_Python/rl_trainer/models/snakes_3v3/run24/trained_model/actor_4000.pth"
    model_enemy_path =  "/home/j-zhong/work_place/TD3_SAC_PPO_multi_Python/rl_trainer/models/snakes_3v3/run24/trained_model/enemy.pth"
        ...
        while True:
            ...
            """if done:  #结束
                if np.sum(episode_reward[:3]) > np.sum(episode_reward[3:]): #AI赢
                    step_reward = get_reward(info, ctrl_agent_index, reward, punishiment_lock,score=1)
                elif np.sum(episode_reward[:3]) < np.sum(episode_reward[3:]):   #random赢
                    step_reward = get_reward(info, ctrl_agent_index, reward, punishiment_lock,score=2)
                else:
                    step_reward = get_reward(info, ctrl_agent_index, reward,punishiment_lock, score=0) #平
            ...
            if args.episode_length <= step: 
                ...
                if rank == 0:
                    ...
                    temp_block = copy.deepcopy(experiment_share_1)
                    ...
                    throughout = len(temp_block)
                    shared_model.push_multi(temp_block)
                    ...
                    torch.save(model_enemy.state_dict(), model_enemy_path)
                    ...
                    if episode % (rank+1) == 0 and episode >30: #200
                        error = True
                        while error :
                            try:
                ...
                if rank == 0:
                    ...
                    wandb.log({"log_alpha":shared_model.get_log_alpha().item(),"a_loss":a_loss,"c_loss":c_loss,
                ...
                if rank == 0 and len(history_success)>=200 and sum(history_success[-200:])>110:
                    print("-------------------------rank",rank,"----------------------------")
                    print("-------------------------Update Enemy!!!----------------------------")
                    print("Success Rate",(sum(history_success[-200:])/200.0)*100,"%")
...
if __name__ == '__main__':# big 21--24   #smaller 22 -- 23 #bigger 25
    ...
    parser.add_argument('--buffer_size', default=int(2e6), type=int) #1e6
    ...
    parser.add_argument('--batch_size', default= 512, type=int)#32768  16384 8192 4096
    ...
    for rank in range(args.processes):# #rank 编号
        ...
        p = mp.Process(target=train, args=( rank, args,device,log_dir,run_dir,shared_lock,
                            model_share ,experiment_share_1 , None,None))#experiment_share_2 ,experiment_share_3
