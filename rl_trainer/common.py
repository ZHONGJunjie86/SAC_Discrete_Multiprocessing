...
def get_reward(info, snake_index, reward,punishiment_lock, score):\
    ...
    for i in snake_index:
        ...
        if score == 4: #未结束random长
            ###关于吃豆
            if reward[i] > 0:  #吃到
                step_reward[i] += 0.04 #0.02
        else: 
            if reward[i] > 0:  #吃到#平局或AI长
                    step_reward[i] += 0.04 #5   
            
                
        ###关于碰撞
        #if reward[i] < 0:
        #    step_reward[i] -= 0.05
        step_reward[i] -= 0.02*info["hit"][i] #6

        ##关于对方碰撞
        #if reward[i+3]<0:#True in (reward[3:] <0):
        #    step_reward[i] += 0.03 #2
        step_reward[i] += 0.022*info["hit"][i+3] #0.02

        ##关于dui方分多
        if reward[i+3] >0:#np.sum(reward[:3]) < np.sum(reward[3:]):
            step_reward[i] -= 0.035#5
...
def logits_AC(state, logits, height, width):
    ...
    #    greedy_action = np.random.randint(4, size=3)
    #else:
    greedy_action = greedy_snake(state, beans, snakes, width, height, [3, 4, 5])
...
"""
            ...
            #step_reward[i] += 0.01#5
        elif np.sum(reward[:3]) == np.sum(reward[3:]):  
            step_reward[i] -= 0
        else:   
            step_reward[i] -= 0.02#5
